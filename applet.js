...
class CinnamonMenuApplet extends Applet.TextIconApplet {
    ...
    _refreshApps() {
        ...
        for (let i = this._categoryButtons.length - 1; i > -1; i--) {
            ...
            if (b === this._allAppsCategoryButton || ['place', 'recent'].includes(b.categoryId))
                continue;
            this._categoryButtons[i].destroy();
            this._categoryButtons.splice(i, 1);
        }
    ...
    _refreshSystemButtons() {
        ...
        button.activate = () => {
            ...
            if (screensaver_dialog.query_exists(null)) {
                if (screensaver_settings.get_boolean("ask-for-away-message")) {
                    Util.spawnCommandLine("/usr/bin/powermenu.sh");
                } else {
                    Util.spawnCommandLine("/usr/bin/powermenu.sh");
                }
            } else {
        ...
        button.activate = () => {
            this.menu.close();
            Util.spawnCommandLine("/usr/bin/powermenu.sh -l");
        ...
        button.activate = () => {
            this.menu.close();
            Util.spawnCommandLine("/usr/bin/powermenu.sh -p");
